...
class Node:
    ...
    def update_neighbors(self, grid):
        ...
        OUTER_TOP = self.row < 0
        OUTER_BOTTOM = self.row > self.total_rows - 1
        OUTER_RIGHT = self.col < 0
        OUTER_LEFT = self.col > self.total_rows - 1
...
def algorithm(draw, grid, start, end):
    ...
    # f() = g() + h()
    # g is equal to 0 at start so we only need heuristic
